        ...
        <div class="space-y-2">
            @foreach ($notifications as $notification)
                ...
                <form method="POST" action="{{ route('notifications.read', $notification->id) }}">
                    @csrf
                    <button type="submit"
                        class="flex w-full items-start gap-3 rounded-2xl bg-white p-4 text-left shadow-sm ring-1 ring-gray-200 transition hover:ring-brand-purple-300 dark:bg-slate-900 dark:ring-slate-700 {{ $notification->read_at ? '' : 'bg-brand-purple-50/40 dark:bg-brand-purple-500/[0.05]' }}">
