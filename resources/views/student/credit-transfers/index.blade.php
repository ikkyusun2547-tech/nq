...
<div class="mx-auto max-w-md" x-data="{ showForm: {{ $errors->any() ? 'true' : 'false' }}, position: '{{ old('position', '') }}', positionHours: @js(\App\Models\CreditTransferRequest::POSITION_HOURS) }" @change="if ($event.target.name === 'position') position = $event.target.value">
    <div class="mb-4 flex items-center justify-between gap-3 rounded-3xl brand-gradient p-6 shadow-soft-lg">
        <div>
            ...
            <h1 class="mt-1 text-lg font-bold text-white">{{ __('เทียบโอนชั่วโมงจากตำแหน่ง') }}</h1>
        </div>
    ...
    <button @click="showForm = ! showForm"
        ...
        x-text="showForm ? '{{ __('ปิดฟอร์ม') }}' : '+ {{ __('ยื่นคำร้องเทียบโอนชั่วโมง') }}'">
    </button>
    ...
    <div x-show="showForm" x-cloak class="mb-6 rounded-3xl glass-card p-5 shadow-soft-lg">
        ...
        <form method="POST" action="{{ route('credit-transfers.store') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                />
                <p class="mt-1.5 text-xs text-brand-green-600 dark:text-brand-green-400" x-show="position" x-text="'{{ __('ชั่วโมงที่จะได้รับ:') }} ' + (positionHours[position] || 0) + ' {{ __('ชม.') }}'"></p>
            </div>
            <div>
