...
<div class="mx-auto max-w-md" x-data="{ showForm: false }">
    <div class="mb-4 flex items-center justify-between gap-3 rounded-3xl brand-gradient p-6 shadow-soft-lg">
        <div>
            <p class="text-xs font-medium uppercase tracking-[0.2em] text-violet-200/70">เทียบชั่วโมงกิจกรรม</p>
            <h1 class="mt-1 text-lg font-bold text-white">คำร้องกิจกรรมภายนอก</h1>
        </div>
    ...
    <div x-show="showForm" x-cloak class="mb-6 rounded-3xl glass-card p-5 shadow-soft-lg">
        ...
        <form method="POST" action="{{ route('external-activities.store') }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-600">ชื่อกิจกรรม</label>
                <input type="text" name="title" value="{{ old('title') }}" required class="{{ $inputClass }}">
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-600">หน่วยงานผู้จัด</label>
                <input type="text" name="organization" value="{{ old('organization') }}" required class="{{ $inputClass }}">
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-600">วันที่จัดกิจกรรม</label>
                <input type="date" name="activity_date" value="{{ old('activity_date') }}" max="{{ date('Y-m-d') }}" required class="{{ $inputClass }}">
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-600">หมวดหมู่กิจกรรม</label>
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-600">จำนวนชั่วโมงที่ขอเทียบ</label>
                <input type="number" name="hours_requested" value="{{ old('hours_requested') }}" min="1" max="200" required class="{{ $inputClass }}">
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-600">ภาพหลักฐาน (เกียรติบัตร/ภาพเข้าร่วม, ไม่เกิน 2MB)</label>
            ...
            <div class="rounded-2xl glass-card p-4 shadow-soft">
                <div class="flex items-start justify-between gap-2">
                    <div>
                        ...
                        <p class="text-xs text-slate-400">{{ $categoryLabels[$req->activity_category] }} · {{ $req->hours_requested }} ชม.</p>
                    </div>
                ...
                @if ($req->status === 'rejected' && $req->reject_reason)
                    <p class="mt-2 rounded-xl bg-red-50 px-3 py-2 text-xs text-red-600">เหตุผล: {{ $req->reject_reason }}</p>
                @endif
            </div>
        @empty
            <p class="py-8 text-center text-sm text-slate-400">ยังไม่มีคำร้องกิจกรรมภายนอก</p>
        @endforelse
    </div>
