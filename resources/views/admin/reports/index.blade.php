...
<div class="mx-auto max-w-3xl">
    <div class="mb-6 rounded-3xl brand-gradient p-6 shadow-soft-lg sm:p-8">
        ...
        <h1 class="mt-1 text-xl font-bold text-white sm:text-2xl">{{ __('รายงาน') }}</h1>
    </div>

    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <a href="{{ route('admin.reports.clearance') }}"
            class="flex flex-col gap-2 rounded-2xl glass-card p-5 shadow-soft transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
            <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-green-50 text-brand-green-600 dark:bg-brand-green-500/10 dark:text-brand-green-400">
                <svg class="h-4.5 w-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347"/></svg>
            </span>
            <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ __('รายชื่อนักศึกษาปี 4 ที่ผ่านเกณฑ์') }}</h2>
            <p class="text-xs text-slate-400 dark:text-slate-500">{{ __('PDF สำหรับสำนักทะเบียน') }}</p>
        </a>

        <a href="{{ route('admin.reports.faculty-participation') }}"
            class="flex flex-col gap-2 rounded-2xl glass-card p-5 shadow-soft transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
            <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-purple-50 text-brand-purple-600 dark:bg-brand-purple-500/10 dark:text-brand-purple-400">
            ...
            <p class="text-xs text-slate-400 dark:text-slate-500">{{ __('Excel — จำนวนนักศึกษา, อัตราผ่านเกณฑ์, ชั่วโมง/กิจกรรมเฉลี่ยต่อคณะ') }}</p>
        </a>
    </div>
</div>
@endsection
