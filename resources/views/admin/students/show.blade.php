...
@php
    ...
    $attendanceLabel = ['auto_approved' => 'อนุมัติแล้ว', 'flagged' => 'ถูกตรวจสอบ', 'rejected' => 'ไม่อนุมัติ'];
    $externalBadge = [
        'pending' => 'bg-amber-50 text-amber-700',
        'approved' => 'bg-brand-green-50 text-brand-green-700',
        'rejected' => 'bg-red-50 text-red-700',
    ];
    $externalLabel = ['pending' => 'รอตรวจสอบ', 'approved' => 'อนุมัติแล้ว', 'rejected' => 'ปฏิเสธแล้ว'];
@endphp
...
<div class="mx-auto max-w-4xl">
    <div class="relative overflow-hidden rounded-3xl brand-gradient p-6 text-white shadow-soft-lg sm:p-8">
        ...
        <div class="relative mt-6 flex flex-wrap items-center justify-between gap-2 border-t border-white/10 pt-4">
            <span class="font-mono text-xs tracking-widest text-violet-200/60">รหัส {{ $student->student_id ?? '-' }}</span>
            <span class="text-xs text-violet-200/60">{{ $student->program_type === 'special' ? 'ภาคพิเศษ (กศ.บป.)' : 'ภาคปกติ' }}</span>
    ...
    <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div class="rounded-2xl glass-card p-5 shadow-soft">
            <h2 class="mb-3 text-sm font-semibold text-slate-900">ประวัติเช็กชื่อล่าสุด</h2>
            <div class="space-y-2.5">
                ...
                @empty
                    <p class="py-6 text-center text-sm text-slate-400">ยังไม่มีประวัติเช็กชื่อ</p>
                @endforelse
            </div>
        </div>

        <div class="rounded-2xl glass-card p-5 shadow-soft">
            <h2 class="mb-3 text-sm font-semibold text-slate-900">คำร้องกิจกรรมภายนอกล่าสุด</h2>
            <div class="space-y-2.5">
                @forelse ($externalRequests as $req)
                    <div class="flex items-start justify-between gap-2 rounded-xl bg-white/60 p-3 text-sm shadow-soft">
                        <div>
                            <p class="font-medium text-slate-800">{{ $req->title }}</p>
                            <p class="text-xs text-slate-400">{{ $req->activity_date->format('d/m/Y') }} · {{ $req->hours_requested }} ชม.</p>
                        </div>
                ...
                @empty
                    <p class="py-6 text-center text-sm text-slate-400">ยังไม่มีคำร้องกิจกรรมภายนอก</p>
                @endforelse
            </div>
        </div>
    </div>
