...
@php
    $categoryMeta = [
        'culture' => ['label' => __('ทำนุบำรุงศิลปวัฒนธรรม'), 'bar' => 'bg-sky-400', 'dot' => 'bg-sky-400'],
        'academic' => ['label' => __('วิชาการ'), 'bar' => 'bg-brand-green-500', 'dot' => 'bg-brand-green-500'],
        'sports' => ['label' => __('กีฬาและส่งเสริมสุขภาพ'), 'bar' => 'bg-amber-400', 'dot' => 'bg-amber-400'],
        'volunteer' => ['label' => __('จิตอาสา/บำเพ็ญประโยชน์'), 'bar' => 'bg-brand-purple-500', 'dot' => 'bg-brand-purple-500'],
        'ethics' => ['label' => __('คุณธรรมจริยธรรม'), 'bar' => 'bg-fuchsia-400', 'dot' => 'bg-fuchsia-400'],
    ];
    $statusBadge = [
...
<div class="mx-auto max-w-[90rem]" x-data="{ trendHover: null }">
    ...
    <form method="GET" action="{{ route('admin.dashboard') }}" class="mt-4 flex flex-wrap items-center gap-3">
    ...
    @php
        $overviewCards = [
            ['label' => __('นักศึกษาทั้งหมด'), 'value' => $stats['total_students'], 'icon' => 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z', 'cardBg' => 'bg-brand-purple-50 dark:bg-brand-purple-500/10', 'cardBorder' => 'border-brand-purple-100 dark:border-brand-purple-500/20', 'iconBg' => 'bg-brand-purple-500'],
            ['label' => __('กิจกรรมที่เปิดอยู่'), 'value' => $stats['open_activities'], 'suffix' => __('/ :total ทั้งหมด', ['total' => $stats['total_activities']]), 'icon' => 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5', 'href' => route('admin.activities.index'), 'cardBg' => 'bg-sky-50 dark:bg-sky-500/10', 'cardBorder' => 'border-sky-100 dark:border-sky-500/20', 'iconBg' => 'bg-sky-500'],
            ['label' => __('เช็คชื่อเดือนนี้'), 'value' => $stats['checkins_this_month'], 'delta' => $checkinDelta, 'icon' => 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z', 'cardBg' => 'bg-brand-green-50 dark:bg-brand-green-500/10', 'cardBorder' => 'border-brand-green-100 dark:border-brand-green-500/20', 'iconBg' => 'bg-brand-green-500'],
            ['label' => __('นักศึกษาปี 4 พร้อมจบ'), 'value' => $stats['graduating_cleared'], 'suffix' => __('/ :total คน', ['total' => $stats['total_year4_students']]), 'icon' => 'M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5', 'href' => route('admin.reports.clearance', ['year' => 4]), 'cardBg' => 'bg-teal-50 dark:bg-teal-500/10', 'cardBorder' => 'border-teal-100 dark:border-teal-500/20', 'iconBg' => 'bg-teal-500'],
        ];

        $actionCards = [
            ['label' => __('การเช็คชื่อติดธงแดง'), 'value' => $stats['flagged_attendances'], 'icon' => 'M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z', 'href' => route('admin.attendance.flagged'), 'urgent' => $stats['flagged_attendances'] > 0, 'cardBg' => 'bg-red-50 dark:bg-red-500/10', 'cardBorder' => 'border-red-100 dark:border-red-500/20', 'iconBg' => 'bg-red-500', 'pulseBg' => 'bg-red-400', 'pulseDot' => 'bg-red-500', 'ringClass' => 'ring-1 ring-red-300 dark:ring-red-500/40'],
            ['label' => __('คำร้องภายนอกรออนุมัติ'), 'value' => $stats['pending_external_requests'], 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', 'href' => route('admin.external-activities.index'), 'urgent' => $stats['pending_external_requests'] > 0, 'cardBg' => 'bg-amber-50 dark:bg-amber-500/10', 'cardBorder' => 'border-amber-100 dark:border-amber-500/20', 'iconBg' => 'bg-amber-500', 'pulseBg' => 'bg-amber-400', 'pulseDot' => 'bg-amber-500', 'ringClass' => ''],
            ['label' => __('เทียบโอนตำแหน่งรออนุมัติ'), 'value' => $stats['pending_credit_transfers'], 'icon' => 'M4.5 6.75h15m-15 0A2.25 2.25 0 002.25 9v6a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 15V9a2.25 2.25 0 00-2.25-2.25m-15 0V5.25A2.25 2.25 0 016.75 3h10.5a2.25 2.25 0 012.25 2.25v1.5m-15 0h15', 'href' => route('admin.credit-transfers.index'), 'urgent' => $stats['pending_credit_transfers'] > 0, 'cardBg' => 'bg-amber-50 dark:bg-amber-500/10', 'cardBorder' => 'border-amber-100 dark:border-amber-500/20', 'iconBg' => 'bg-amber-500', 'pulseBg' => 'bg-amber-400', 'pulseDot' => 'bg-amber-500', 'ringClass' => ''],
        ];

        $totalPending = $stats['flagged_attendances'] + $stats['pending_external_requests'] + $stats['pending_credit_transfers'];

        $renderKpiCard = function ($card) {
            $tag = isset($card['href']) ? 'a' : 'div';
            $attrs = isset($card['href']) ? 'href="'.$card['href'].'"' : '';
            $classes = 'relative overflow-hidden rounded-2xl border p-4 shadow-soft transition '.$card['cardBg'].' '.$card['cardBorder']
                .(isset($card['href']) ? ' hover:-translate-y-0.5 hover:shadow-lg' : '')
                .(($card['urgent'] ?? false) ? ' '.($card['ringClass'] ?? '') : '');

            return compact('tag', 'attrs', 'classes');
        };
    @endphp
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
        @foreach ($overviewCards as $card)
            @php $r = $renderKpiCard($card); @endphp
            <{{ $r['tag'] }} {!! $r['attrs'] !!} class="{{ $r['classes'] }}">
                <div class="flex items-center justify-between">
                    <span class="flex h-10 w-10 items-center justify-center rounded-xl {{ $card['iconBg'] }} text-white shadow-soft">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="{{ $card['icon'] }}"/></svg>
                    </span>
                    @if (isset($card['delta']))
                        <span @class([
                            'inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[0.68rem] font-semibold tabular-nums',
                            'bg-brand-green-100 text-brand-green-700 dark:bg-brand-green-500/15 dark:text-brand-green-400' => $card['delta'] >= 0,
                            'bg-red-100 text-red-600 dark:bg-red-500/15 dark:text-red-400' => $card['delta'] < 0,
                        ])>
                ...
                <p class="mt-3 text-2xl font-bold text-gray-900 dark:text-slate-100">{{ number_format($card['value']) }}<span class="text-xs font-normal text-gray-500 dark:text-slate-400">{{ $card['suffix'] ?? '' }}</span></p>
                <p class="mt-0.5 text-xs font-medium text-gray-500 dark:text-slate-400">{{ $card['label'] }}</p>
            </{{ $r['tag'] }}>
        @endforeach
    </div>
    ...
    <div class="mb-3 mt-8 flex items-center justify-between px-1">
        ...
        @if ($totalPending > 0)
            <span class="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[0.68rem] font-semibold text-amber-700 dark:bg-amber-500/15 dark:text-amber-400">
                {{ __('รวม :count รายการ', ['count' => $totalPending]) }}
            </span>
        @else
            <span class="inline-flex items-center gap-1 rounded-full bg-brand-green-100 px-2 py-0.5 text-[0.68rem] font-semibold text-brand-green-700 dark:bg-brand-green-500/15 dark:text-brand-green-400">
                {{ __('ไม่มีรายการค้าง') }}
            </span>
        @endif
    </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    @if ($card['urgent'] ?? false)
                        <span class="relative flex h-2.5 w-2.5">
                            <span class="absolute inline-flex h-full w-full animate-ping rounded-full {{ $card['pulseBg'] }} opacity-75"></span>
                            <span class="relative inline-flex h-2.5 w-2.5 rounded-full {{ $card['pulseDot'] }}"></span>
                        </span>
                    @endif
                </div>
                <p class="mt-3 text-2xl font-bold text-gray-900 dark:text-slate-100">{{ number_format($card['value']) }}<span class="text-xs font-normal text-gray-500 dark:text-slate-400">{{ $card['suffix'] ?? '' }}</span></p>
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-5">
        <x-section-card icon="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" :title="__('ภาพรวมความสำเร็จนักศึกษาปี 4')" class="lg:col-span-2">
        ...
        <x-section-card icon="M4 20V10M12 20V4M20 20V14" :title="__('ชั่วโมงกิจกรรมแยกตามหมวดหมู่')" class="lg:col-span-3">
            <x-slot:action>
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <x-section-card icon="M3 17l6-6 4 4 8-8M21 7v6h-6" :title="$academicYear !== '' ? __('แนวโน้มการเช็คชื่อรายเดือน') : __('แนวโน้มการเช็คชื่อ 6 เดือนล่าสุด')" class="lg:col-span-2">
            <x-slot:action>
                <span class="text-xs text-slate-400 dark:text-slate-500">{{ $academicYearScopeLabel }}</span>
            </x-slot:action>
            ...
            <div class="relative">
                <svg viewBox="0 0 {{ $chartW }} 146" class="w-full overflow-visible text-brand-purple-500 dark:text-brand-purple-400" preserveAspectRatio="none" style="height: 10.5rem;">
                    ...
                    @foreach ($coords as $i => $c)
                        <circle cx="{{ $c['x'] }}" cy="{{ $c['y'] }}" r="{{ $i === $coords->count() - 1 ? 4 : 3 }}" fill="currentColor" stroke="white" stroke-width="2" class="dark:[stroke:#1e1b33]"/>
                ...
                @foreach ($coords as $i => $c)
                    <div x-show="trendHover === {{ $i }}" x-cloak x-transition.opacity.duration.100ms
                        class="pointer-events-none absolute z-10 -translate-x-1/2 -translate-y-full rounded-lg bg-brand-purple-950 px-2.5 py-1.5 text-center text-xs font-medium text-white shadow-soft-lg"
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <x-section-card icon="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" :title="__('กิจกรรมที่กำลังจะถึง')">
            <x-slot:action>
        ...
        <x-section-card icon="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z" :title="__('ประวัติการตรวจสอบล่าสุด')">
            <x-slot:action>
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-4">
        @php
            $quickActions = [
                ['route' => 'admin.activities.index', 'label' => __('จัดการกิจกรรม'), 'desc' => __('สร้าง/แก้ไขกิจกรรม กำหนดสิทธิ์ผู้เข้าร่วม'), 'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4', 'tint' => 'bg-brand-green-50 text-brand-green-600 dark:bg-brand-green-500/10 dark:text-brand-green-400'],
                ['route' => 'admin.external-activities.index', 'label' => __('คำร้องกิจกรรมภายนอก'), 'desc' => __('ตรวจสอบและอนุมัติ/ปฏิเสธคำร้อง'), 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', 'tint' => 'bg-brand-purple-50 text-brand-purple-600 dark:bg-brand-purple-500/10 dark:text-brand-purple-400'],
                ['route' => 'admin.credit-transfers.index', 'label' => __('เทียบโอนชั่วโมงจากตำแหน่ง'), 'desc' => __('ตรวจสอบ อนุมัติ และให้เครดิตชั่วโมง'), 'icon' => 'M4.5 6.75h15m-15 0A2.25 2.25 0 002.25 9v6a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 15V9a2.25 2.25 0 00-2.25-2.25m-15 0V5.25A2.25 2.25 0 016.75 3h10.5a2.25 2.25 0 012.25 2.25v1.5m-15 0h15', 'tint' => 'bg-brand-purple-50 text-brand-purple-600 dark:bg-brand-purple-500/10 dark:text-brand-purple-400'],
                ['route' => 'admin.late-checkins.index', 'label' => __('เช็คชื่อย้อนหลัง'), 'desc' => __('ตรวจสอบคำร้องขอเช็คชื่อย้อนหลัง'), 'icon' => 'M12 6v6l4 2M21 12a9 9 0 11-18 0 9 9 0 0118 0z', 'tint' => 'bg-sky-50 text-sky-600 dark:bg-sky-500/10 dark:text-sky-400'],
                ['route' => 'admin.students.index', 'label' => __('ข้อมูลนักศึกษา'), 'desc' => __('ค้นหา/กรอง ดูรายบุคคล'), 'icon' => 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z', 'tint' => 'bg-brand-purple-50 text-brand-purple-600 dark:bg-brand-purple-500/10 dark:text-brand-purple-400'],
                ['route' => 'admin.announcements.create', 'label' => __('ส่งประกาศ'), 'desc' => __('แจ้งเตือนนักศึกษาทั้งหมดหรือกลุ่มที่เลือก'), 'icon' => 'M10.34 15.84c-.688-.06-1.386-.09-2.09-.09H7.5a4.5 4.5 0 110-9h.75c.704 0 1.402-.03 2.09-.09m0 9.18c2.31.192 4.594.591 6.81 1.17a48.11 48.11 0 003.65-8.35 48.11 48.11 0 00-3.65-8.35 48.51 48.51 0 00-6.81 1.17m0 6.42a48.517 48.517 0 010-6.42', 'tint' => 'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400'],
                ['route' => 'admin.reports.index', 'label' => __('รายงาน'), 'desc' => __('PDF ยื่นจบ และสรุปรายคณะ (Excel)'), 'icon' => 'M12 4v16m8-8H4', 'tint' => 'bg-brand-green-50 text-brand-green-600 dark:bg-brand-green-500/10 dark:text-brand-green-400'],
            ];
            if (auth()->user()->role === 'super_admin') {
                $quickActions[] = ['route' => 'admin.users.index', 'label' => __('ผู้ใช้งานและสิทธิ์'), 'desc' => __('เลื่อน/ลดสิทธิ์แอดมิน ระงับบัญชี'), 'icon' => 'M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z', 'tint' => 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'];
            }
        @endphp
        @foreach ($quickActions as $action)
            <a href="{{ route($action['route']) }}" class="group rounded-2xl glass-card p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lg">
                <span class="flex h-10 w-10 items-center justify-center rounded-xl {{ $action['tint'] }} transition group-hover:scale-105">
